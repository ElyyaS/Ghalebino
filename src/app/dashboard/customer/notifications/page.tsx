        ...
        <div className="space-y-2">
          {items.map((n) => (
            <Link
              key={n.id}
              href={n.link ?? "#"}
              className={`block rounded-2xl border bg-white p-4 transition-colors hover:border-brand-300 ${
                n.isRead ? "border-slate-200" : "border-brand-200"
              }`}
            >
              <div className="flex items-center justify-between">
                <p className="font-medium text-slate-900">{n.title}</p>
                {!n.isRead ? <span className="h-2 w-2 rounded-full bg-brand-600" /> : null}
              </div>
